                    ...
                    @foreach (config('app.locales') as $locale)
                        <div role="tabpanel" class="tab-pane fade @if ($locale == app()->getLocale()) active show @endif "
                            ...
                            <div class="form-group">
                                ...
                                @if(isset($section->translate($locale)->locale_additional['cover']))
                                    <input type="hidden" name="{{ $locale }}[old_cover]" value="{{ $section->translate($locale)->locale_additional['cover'] }}">
                                @endif
                            
                                <input type="file" name="{{ $locale }}[cover]" value="">
                                @if(isset($section) && isset($section->translate($locale)->locale_additional['cover']))
                                    <br>
                                    <div class="col-md-8 dfile">
                                        <img src="/{{ config('config.image_path').$section->translate($locale)->locale_additional['cover'] }}" alt="" style="width: 25%">
                                       <span class="deletefile" data-lang="{{ $locale }}"
                                                data-id="{{ $section->id }}" data-token="{{ csrf_token() }}"
                                                data-route="/{{ app()->getLocale() }}/admin/sections/DeleteCover/{{ $section->id }}"
                                                delete="{{ $section->translate($locale)->locale_additional['cover'] }}">
                                                <i class="fa fa-trash" style="color: red; padding:10px"></i>
                                            </span>
                                    </div>
                                @endif
                            </div>
                            <div class="form-group">
                                <label for="{{ $locale }}-alt_text">{{ trans('admin.alt_text') }}</label>
                               
                                <input type="text" name="{{ $locale }}[alt_text]"
                                    value="{{ $section->translate($locale)->locale_additional['alt_text'] ?? '' }}" parsley-trigger="change"
                                    class="form-control"
                                    id="{{ $locale }}-alt_text" >
                            </div>
                        </div>
